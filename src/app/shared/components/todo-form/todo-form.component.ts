...
export class TodoFormComponent implements OnInit, OnChanges {
  ...
  @Output() EmitUpdate: EventEmitter<Itodos> =
    new EventEmitter<Itodos>();
  ...
  ngOnChanges(changes: SimpleChanges): void {
    ...
    if (changes['editObj'] && changes['editObj'].currentValue) {

      this.isinEditMode = true;

      if (this.todoItem && this.isCompleted) {

        this.todoItem.nativeElement.value =
          this.editObj.todoItem;

        this.isCompleted.nativeElement.value =
          this.editObj.isCompleted;
      }
    }
  }
